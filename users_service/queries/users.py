...
class UserRepository:
            ...
            with conn.cursor() as db:
                result = db.execute(
                    """
                        ...
                        RETURNING id;
                        """,
                    [user.username, hashed_password, user.email, user.name],
                )
    ...
    def get_by_username(
        self, username: str
    ) -> Union[UserOutWithPassword, UserError]:
        try:
            with conn.cursor() as db:
                result = db.execute(
                    """
    ...
    def user_query_to_userout(self, query: tuple) -> UserOut:
        return UserOut(
            id=query[0], username=query[1], email=query[2], name=query[3]
        )
            ...
            with conn.cursor() as db:
                result = db.execute(
                    """
                        ...
                        """,
                    [user.password, user.email, user.name, user_id],
                )
                query = result.fetchone()
                return self.user_query_to_userout(query)
        except Exception:

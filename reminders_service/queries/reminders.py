...
class ReminderRepository:
                ...
                for record in query:
                    if record[0] not in new_dict:
                        new_dict[record[0]] = ReminderOut(
                            ...
                            message=MessageRepository.get_one(
                                MessageRepository, record[4]
                            ),
                            sent=record[5],
                            sent_on=record[6],
                            recurring=record[7],
                        ...
                        new_dict[record[0]].recipients.append(
                            RecipientOut(
                                id=record[9],
                                name=record[10],
                                phone=record[11],
    ...
    def update(
        self, user_id: int, reminder_id: int, reminder: ReminderUpdate
    ) -> Union[ReminderUpdate, Error]:
        try:
            # with pool.connection() as conn:
            with conn.cursor() as db:
                result = db.execute(
                ...
                return ReminderUpdate(
                    email_target=query[0],
                    reminder_date=query[1],
                    recurring=query[2],
                )
        except Exception:
    ...
    # HELPER FUNCTIONS
    def reminder_query_to_reminder_out(
        self,
        query: tuple,
        recipient_list: List[RecipientOut],
        message: MessageOut,
    ) -> ReminderOut:
        return ReminderOut(
            id=query[0],
